...
describe("Stubs API", () => {
  it("POST /imposters/:id/stubs adds a stub", async () => {
    ...
    try {
      ...
      const res = await handler(new Request(`http://localhost/imposters/${imposter.id}/stubs`, json({
  ...
  it("POST /imposters/:id/stubs returns 404 for non-existent imposter", async () => {
    const { handler, dispose } = makeHandler()
    try {
      const res = await handler(new Request("http://localhost/imposters/nonexistent/stubs", json({
  ...
  it("GET /imposters/:id/stubs lists stubs", async () => {
    ...
    try {
      ...
      await handler(new Request(`http://localhost/imposters/${imposter.id}/stubs`, json({
        responses: [{ status: 200 }]
      })))
      await handler(new Request(`http://localhost/imposters/${imposter.id}/stubs`, json({
  ...
  it("PUT /imposters/:id/stubs/:stubId updates a stub", async () => {
    ...
    try {
      ...
      const createRes = await handler(new Request(`http://localhost/imposters/${imposter.id}/stubs`, json({
        responses: [{ status: 200 }]
      })))
      const stub = await createRes.json()

      const res = await handler(new Request(`http://localhost/imposters/${imposter.id}/stubs/${stub.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  it("PUT /imposters/:id/stubs/:stubId returns 404 for non-existent stub", async () => {
    ...
    try {
      ...
      const res = await handler(new Request(`http://localhost/imposters/${imposter.id}/stubs/nonexistent`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
